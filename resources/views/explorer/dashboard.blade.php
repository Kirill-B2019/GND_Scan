...
@section('content')
    ...
    @if($gndConfigured)
        ...
        <div id="dashboard-cards" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-10">
            <div class="rounded-xl border explorer-border explorer-bg-card p-4 explorer-card-hover transition-colors">
                <div class="explorer-text-muted text-xs uppercase tracking-wide">Блоков</div>
                <div id="dashboard-blocks-count" class="text-xl font-mono font-semibold explorer-primary mt-1">{{ number_format($metrics['blocks_count'] ?? $metrics['total_blocks'] ?? 0, 0, ',', ' ') }}</div>
            </div>
            <div class="rounded-xl border explorer-border explorer-bg-card p-4 explorer-card-hover transition-colors">
                <div class="explorer-text-muted text-xs uppercase tracking-wide">Транзакций</div>
                <div id="dashboard-tx-count" class="text-xl font-mono font-semibold explorer-primary mt-1">{{ number_format($metrics['transactions_count'] ?? $metrics['total_transactions'] ?? 0, 0, ',', ' ') }}</div>
            </div>
            <div class="rounded-xl border explorer-border explorer-bg-card p-4 explorer-card-hover transition-colors">
                <div class="explorer-text-muted text-xs uppercase tracking-wide">TPS</div>
                <div id="dashboard-tps" class="text-xl font-mono font-semibold explorer-primary mt-1">{{ $metrics['tps'] ?? $metrics['transactions_per_second'] ?? '—' }}</div>
            </div>
            <div class="rounded-xl border explorer-border explorer-bg-card p-4 explorer-card-hover transition-colors">
                ...
                <div id="dashboard-avg-fee" class="text-lg font-mono explorer-text mt-1">{{ $fee !== null && $fee !== '' ? $fee . ' GND' : '—' }}</div>
            </div>
            <div class="rounded-xl border explorer-border explorer-bg-card p-4 explorer-card-hover transition-colors col-span-2 sm:col-span-1">
                <div class="explorer-text-muted text-xs uppercase tracking-wide">Валидаторы</div>
                <div id="dashboard-validators" class="text-xl font-mono font-semibold explorer-primary mt-1">{{ $metrics['validators_count'] ?? '—' }}</div>
            </div>
        </div>
    ...
    <div class="grid lg:grid-cols-2 gap-8">
        <section>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-semibold explorer-text">Последние блоки</h2>
                <a href="{{ route('explorer.stats') }}" class="text-sm explorer-primary explorer-link">Все</a>
            </div>
            <div id="dashboard-blocks-list" class="rounded-xl border explorer-border explorer-bg-card overflow-hidden">
                @forelse($blocks as $block)
                    ...
                    @endphp
                    <a href="{{ route('explorer.block.show', ['number' => $id]) }}" class="flex items-center justify-between px-4 py-3 border-b explorer-divider last:border-0 hover:bg-[var(--color-surface-dark)] transition-colors">
                        <span class="font-mono explorer-primary font-medium">#{{ $id }}</span>
                        <span class="explorer-text-muted text-sm">{{ $txCount }} tx</span>
                        <span class="block-time-ago explorer-text-muted text-sm" data-block-ts="{{ $ts ? \Carbon\Carbon::parse($ts)->toIso8601String() : '' }}">{{ $ts ? \Carbon\Carbon::parse($ts)->diffForHumans() : '—' }}</span>
                    </a>
                @empty
        ...
        <section>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-semibold explorer-text">Последние транзакции</h2>
                <a href="{{ route('explorer.stats') }}" class="text-sm explorer-primary explorer-link">Все</a>
            </div>
            <div id="dashboard-tx-list" class="rounded-xl border explorer-border explorer-bg-card overflow-hidden">
                @forelse($transactions as $tx)
                    ...
                    @endphp
                    <a href="{{ route('explorer.transaction.show', ['hash' => $hash]) }}" class="flex items-center justify-between gap-2 px-4 py-3 border-b explorer-divider last:border-0 hover:bg-[var(--color-surface-dark)] transition-colors flex-wrap sm:flex-nowrap">
                        <span class="font-mono text-sm explorer-text-muted-4 truncate max-w-[140px] sm:max-w-[180px]" title="{{ $hash }}">{{ $shortHash }}</span>
                        ...
                        <span class="text-xs shrink-0 {{ $status === 'confirmed' ? 'text-emerald-400' : ($status === 'failed' ? 'text-red-400' : 'explorer-primary') }}">{{ $status === 'confirmed' ? '✓' : ($status === 'failed' ? '✗' : '…') }}</span>
                    </a>
                @empty
            ...
            (function() {
                ...
                function renderBlocks(blocks) {
                    ...
                    return blocks.map(function(b) {
                        ...
                        return '<a href="' + BLOCK_URL + '/' + id + '" class="flex items-center justify-between px-4 py-3 border-b explorer-divider last:border-0 explorer-row-hover transition-colors">' +
                            '<span class="font-mono explorer-primary font-medium">#' + id + '</span>' +
                            '<span class="explorer-text-muted text-sm">' + txCount + ' tx</span>' +
                            '<span class="block-time-ago explorer-text-muted text-sm" data-block-ts="' + tsAttr + '">' + timeAgo(ts) + '</span></a>';
                    }).join('');
                }
